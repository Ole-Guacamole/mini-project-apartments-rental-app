...
function ApartmentCard(props) {
  ...
  const renderLabels = () => {
    ...
    if (apartment.review_scores_rating >= 70) {
      labels.push(
        <span key="top-review-score" className="label top-review-score">
          Top Reviews
        </span>
      );
    }
    ...
    if (apartment.room_type === "Entire home/apt") {
      labels.push(
        <span key="entire-home" className="label entire-home">
          Entire Apartment
        </span>
      );
    }
    ...
    if (apartment.cancellation_policy === "flexible") {
      labels.push(
        <span
          key="flexible-cancellation"
          className="label flexible-cancellation"
        >
  ...
  return (
    <div className="apartment-card">
      ...
      <p className="dachzeile">
        ...
        <p className="numbers-bar">
          Max {apartment.accommodates} persons | Bedrooms: {apartment.bedrooms}{" "}
          | Beds: {apartment.beds} | Bathrooms: {apartment.bathrooms}
        </p>
        </Link>
        ...
        <div className="card-buttons">
          ...
          <button
            onClick={() => (
              handleFavourite(apartment.id), setFavourited(!favourited)
            )}
            type="button"
            className="card-button"
          >
